...
class SelectPatternTest < Minitest::Test
  ...
  def test_pick_words_ending_in_ing
    ...
    selected = []
    words.each{|word| selected << word if word[-3, 3] == "ing"}
  ...
  def test_pick_floats
    ...
    floats = []
    numbers.each{|number| floats << number if number.class == Float}
  ...
  def test_pick_arrays
    ...
    arrays = []
    elements.each {|element| arrays.push(element) if element.class == Array}
